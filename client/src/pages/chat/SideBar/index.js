...
const ListItem = ({
  ...
  return (
    <div
      onClick={handleClick}
      className={`flex items-center gap-4 px-4 py-3 border-b hover:bg-hover cursor-pointer ${
        isActive ? "bg-hover" : ""
      }`}
    >
      ...
      <div className="flex flex-col items-end justify-between  gap-2">
        ...
        {unseenMessage && (
          <div className="rounded-full w-5 h-5 text-white px-1 bg-red-400 flex items-center justify-center text-xs">
            <p>{unseenMessage}</p>
          </div>
        )}
      </div>
...
const SideBar = ({ onClose }) => {
  ...
  const { data, refetch } = useQuery({
    queryKey: ["conversationList", filter],
    queryFn: ({ queryKey }) => getUserConversation(queryKey[1]),
  });
  ...
  return (
    <div className="border relative h-full">
      ...
      <div>
        ...
        <div className="sm:hidden flex px-3 border-b py-2 items-center justify-between">
        ...
        {/* Desktop header */}
        <div className="hidden sm:flex px-3 border-b py-2 items-center justify-between">
          <div
            className="rounded-full cursor-pointer w-10 h-10 flex items-center justify-center overflow-hidden"
        ...
        {/* Conversations title for desktop only */}
        <div className="hidden sm:block border text-center font-semibold text-secondary py-2">
          Conversations
        </div>
      ...
      <div className="overflow-auto [height:83%]">
        {data?.data?.length === 0 && (
          <div className="text-center py-2">No conversation found.</div>
        )}
        {data?.data?.map((converationItem) => (
          <ListItem
            key={converationItem._id}
            {...converationItem}
            isActive={converationItem._id === conversationId}
          />
        ))}
      </div>
    </div>
